...
- **Approach:** Change `options.bundleIdPrefix` to `zinqq`, the app target's `PRODUCT_BUNDLE_IDENTIFIER` to `zinqq.ios`, and the test target's to `zinqq.ios.tests`. Leave Kotlin package names and Android IDs untouched per KTD1.
...
- No `zinqq.spike` identifier remains in `iosApp/project.yml`; Kotlin packages intentionally unchanged.
