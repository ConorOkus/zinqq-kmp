...
All three questions the requirements phase deferred to planning are resolved in the Planning Contract: Esplora access (KTD-5: public `mempool.space/api`, configurable), exact crate versions and the UniFFI surface (KTD-1, U3), and repository/scaffold layout (this repo; see Output Structure). Two execution-time unknowns remain, both non-blocking:

- Whether Megalith requires an access token on `lsps2.get_info`. Deferred to implementation: attempt `token: None` first; on rejection, request access via megalithic.me/contact. This is the longest external lead time — send the contact request during U1, not at U4.
- Megalith's live fee menu, payment-size bounds, and trust-model flag. Not published; read them from the `get_info` response at runtime and enforce client-side (U4).
...
- KTD-5. **Esplora default is `https://mempool.space/api`, held in config with `https://blockstream.info/api` as a one-line fallback.** Rationale: keyless, esplora-compatible, actively maintained; Blockstream's open endpoint is de-prioritized behind their keyed Explorer API. Retry frequency is capped so background sync can't storm the endpoint. Resolves the Esplora question the requirements phase deferred. Update (maintainer, 2026-07-25): the PWA's credentialed Esplora endpoint is available to this spike and swaps in via the same config value once its URL/key are copied from the Zinqq web repo; the keyless default remains until then.
